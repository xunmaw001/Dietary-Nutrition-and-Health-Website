...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"30px 20px ","margin":"20px 7% 0","alignItems":"center","flexWrap":"wrap","background":"url(http://codegen.caihongy.cn/20230206/544f501163914411bf89e5cb898e10ac.png) no-repeat center bottom,linear-gradient(30deg, rgba(255,255,255,1) 0%, rgba(207,193,176,1) 20%, rgba(246,236,223,1) 50%, rgba(207,193,176,1) 80%, rgba(255,255,255,1) 100%),#fff","display":"flex","width":"86%","justifyContent":"center","height":"auto"}'>
      <el-form-item :style='{"alignItems":"center","margin":"0 4px 0 0","display":"flex"}'>
        ...
        <el-input v-model="formSearch.shicaimingcheng" placeholder="食材名称" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"alignItems":"center","margin":"0 4px 0 0","display":"flex"}'>
	  ...
	  <el-button v-if="isAuth('shanshishicai','新增')" :style='{"cursor":"pointer","border":"0","padding":"0px 15px","margin":"0 10px 0 0","color":"#fff","minWidth":"90px","outline":"none","borderRadius":"30px","background":"#c99d6b","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/shanshishicaiAdd')"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
...
<script>
  export default {
    ...
    methods: {
      ...
      getFenlei() {
        this.$http.get('option/shicaileixing/shicaileixing').then(res => {
          if (res.data.code == 0) {
      ...
      getList(page, fenlei, ref = '') {
        ...
        if (this.indexQueryCondition) {
          searchWhere.shicaimingcheng = '%' + this.indexQueryCondition + '%';
          this.indexQueryCondition = '';
        }
        if (this.formSearch.shicaimingcheng != '') searchWhere.shicaimingcheng = '%' + this.formSearch.shicaimingcheng + '%';
        ...
        if (this.curFenlei != '全部') searchWhere.shicaileixing = this.curFenlei;
        this.$http.get('shanshishicai/list', {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        this.$router.push({path: '/index/shanshishicaiDetail', query: {detailObj: JSON.stringify(item)}});
      },
    }
  }
</script>
