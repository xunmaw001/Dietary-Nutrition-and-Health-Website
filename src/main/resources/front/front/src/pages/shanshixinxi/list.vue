...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"30px 20px ","margin":"20px 7% 0","alignItems":"center","flexWrap":"wrap","background":"url(http://codegen.caihongy.cn/20230206/544f501163914411bf89e5cb898e10ac.png) no-repeat center bottom,linear-gradient(30deg, rgba(255,255,255,1) 0%, rgba(207,193,176,1) 20%, rgba(246,236,223,1) 50%, rgba(207,193,176,1) 80%, rgba(255,255,255,1) 100%),#fff","display":"flex","width":"86%","justifyContent":"center","height":"auto"}'>
      <el-form-item :style='{"alignItems":"center","margin":"0 4px 0 0","display":"flex"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>膳食名称</div>
        <el-input v-model="formSearch.shanshimingcheng" placeholder="膳食名称" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"alignItems":"center","margin":"0 4px 0 0","display":"flex"}'>
		<div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>膳食类型</div>
        <el-select v-model="formSearch.shanshileixing" placeholder="请选择膳食类型" :clearable="true">
          <el-option v-for="(item, index) in shanshileixingOptions" :key="index" :label="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item :style='{"alignItems":"center","margin":"0 4px 0 0","display":"flex"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>营养成分</div>
        <el-input v-model="formSearch.yingyangchengfen" placeholder="营养成分" clearable></el-input>
      </el-form-item>
	  ...
	  <el-button v-if="isAuth('shanshixinxi','新增')" :style='{"cursor":"pointer","border":"0","padding":"0px 15px","margin":"0 10px 0 0","color":"#fff","minWidth":"90px","outline":"none","borderRadius":"30px","background":"#c99d6b","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/shanshixinxiAdd')"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
	...
	<div class="list" :style='{"padding":"0 0 20px","margin":"20px 7% 0","background":"none","flex":"1","width":"86%","minWidth":"900px","order":"3"}'>
		...
		<div class="list1 index-pv1" :style='{"padding":"0","margin":"30px 0 0","overflow":"hidden","background":"none","display":"block","width":"100%","height":"auto"}'>
			<div :style='{"margin":"0 3% 20px 0","flexWrap":"wrap","background":"none","display":"flex","width":"30%","position":"relative","float":"left","justifyContent":"space-between","height":"auto"}' v-for="(item, index) in dataList" :key="index" @click="toDetail(item)" class="list-item animation-box">
				...
				<div :style='{"padding":"4px 0","margin":"0 auto","whiteSpace":"nowrap","overflow":"hidden","color":"#fff","textAlign":"center","background":"linear-gradient(270deg, rgba(255,255,255,1) 0%, rgba(207,193,176,1) 20%, rgba(166,147,124,1) 50%, rgba(207,193,176,1) 80%, rgba(255,255,255,1) 100%),#a6937c","width":"86%","lineHeight":"24px","fontSize":"14px","textOverflow":"ellipsis"}' class="name ">膳食名称:{{item.shanshimingcheng}}</div>
				<div :style='{"padding":"4px 0","margin":"0 auto","whiteSpace":"nowrap","overflow":"hidden","color":"#fff","textAlign":"center","background":"linear-gradient(270deg, rgba(255,255,255,1) 0%, rgba(207,193,176,1) 20%, rgba(166,147,124,1) 50%, rgba(207,193,176,1) 80%, rgba(255,255,255,1) 100%),#a6937c","width":"86%","lineHeight":"24px","fontSize":"14px","textOverflow":"ellipsis"}' class="name ">{{item.shanshileixing}}</div>
				<div :style='{"padding":"4px 0","margin":"0 auto","whiteSpace":"nowrap","overflow":"hidden","color":"#fff","textAlign":"center","background":"linear-gradient(270deg, rgba(255,255,255,1) 0%, rgba(207,193,176,1) 20%, rgba(166,147,124,1) 50%, rgba(207,193,176,1) 80%, rgba(255,255,255,1) 100%),#a6937c","width":"86%","lineHeight":"24px","fontSize":"14px","textOverflow":"ellipsis"}' class="name ">{{item.yingyangchengfen}}</div>
			</div>
		</div>
...
<script>
  export default {
    ...
    created() {
      ...
      this.$http.get('option/shanshileixing/shanshileixing').then(res => {
        if (res.data.code == 0) {
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        let searchWhere = {};
        if (this.formSearch.shanshimingcheng != '') searchWhere.shanshimingcheng = '%' + this.formSearch.shanshimingcheng + '%';
        if (this.formSearch.shanshileixing != '') searchWhere.shanshileixing = this.formSearch.shanshileixing;
        if (this.formSearch.yingyangchengfen != '') searchWhere.yingyangchengfen = '%' + this.formSearch.yingyangchengfen + '%';
        this.$http.get('shanshixinxi/list', {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        this.$router.push({path: '/index/shanshixinxiDetail', query: {detailObj: JSON.stringify(item)}});
      },
    }
  }
</script>
